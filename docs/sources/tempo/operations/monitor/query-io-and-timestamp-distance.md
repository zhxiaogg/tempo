...
- `tempo_query_frontend_bytes_returned_total` measures how many bytes of the response payload the query frontend returns to the client per tenant and operation. Currently only trace-by-id (`op="traces"`) populates it.
...
The query frontend emits `tempo_query_frontend_bytes_returned_total` when a trace-by-id request finishes, reflecting the size of the trace actually returned to the client rather than bytes read from storage. Unlike `tempo_query_frontend_bytes_inspected_total`, it is not affected by cache hits.
...
| `tempo_query_frontend_bytes_returned_total` | Counter | `tenant`, `op` | - | On request completion at the query frontend; records the size of the response payload actually returned to the client. | Currently only populated for `op="traces"` (trace-by-id); included on cache hits. |
...
Compare returned bytes to inspected bytes for trace-by-id queries, to see how much of the storage read actually reaches the client:

```promql
sum by (tenant) (rate(tempo_query_frontend_bytes_returned_total{op="traces"}[5m]))
/
sum by (tenant) (rate(tempo_query_frontend_bytes_inspected_total{op="traces"}[5m]))
```
